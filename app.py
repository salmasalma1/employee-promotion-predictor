...
def load_model_artifacts():
    ...
    if not os.path.exists(model_path) or os.path.getsize(model_path) == 0:
        st.error(f"❌ الملف {model_path} غير موجود أو تالف!")
        st.stop()
    ...
    try:
        ...
        # محاولة التحميل كـ Classifier
        model = xgb.XGBClassifier()
        model.load_model(model_path)
        return model, scaler, feature_columns
    except:
        try:
            # محاولة بديلة كـ Booster
            model = xgb.Booster()
            model.load_model(model_path)
            return model, scaler, feature_columns
...
# استدعاء الدالة (تأكد أنها بره الـ def)
...
try:
    # تحويل المدخلات لأرقام وتجهيزها للـ Scaling
...
if st.button("Predict Promotion"):
    try:
        ...
        if prediction == 1:
            ...
            st.write(f"نسبة الثقة: **{prob*100:.2f}%**")
        else:
            st.error(f"### 😔 الترقية غير محتملة حالياً")
            st.write(f"احتمالية الترقية: **{prob*100:.2f}%**")
